...
/**
 * Wymusza użycie szablonów motywu zamiast Elementor Theme Builder.
 *
 * Elementor Pro przechwytuje rendering przez filtr `template_include` i podmienia
 * szablony motywu na własne. Nadpisujemy to z priorytetem 999, aby nasze pliki
 * front-page.php i page-{slug}.php były zawsze użyte.
 */
function sudecka_wataha_force_theme_templates(string $template): string
{
    ...
    if (is_page()) {
        ...
        if ($slug !== '') {
            $page_tpl = get_template_directory() . '/page-' . $slug . '.php';
...
add_filter('template_include', 'sudecka_wataha_force_theme_templates', 999);
